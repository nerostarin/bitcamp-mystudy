...
CREATE TABLE story (
    ...
    conetent        TEXT        NOT NULL COMMENT '내용', -- 내용
...
-- 자주 묻는 질문
CREATE TABLE QnA (
    qna_id  INTEGER NOT NULL COMMENT '질문번호', -- 질문번호
    title   TEXT    NOT NULL COMMENT '제목', -- 제목
...
ALTER TABLE QnA
    MODIFY COLUMN qna_id INTEGER NOT NULL AUTO_INCREMENT COMMENT '질문번호';
...
ALTER TABLE Temporary3
    ADD CONSTRAINT FK_location_TO_Temporary3 -- 지역 -> 메인 스토리
    FOREIGN KEY (
    location_id -- 지역번호
    )
    REFERENCES location ( -- 지역
...
-- 메인 스토리
ALTER TABLE Temporary3
    ADD CONSTRAINT FK_story_TO_Temporary3 -- 스토리 -> 메인 스토리
    FOREIGN KEY (
    story_id -- 스토리번호
    )
    REFERENCES story ( -- 스토리
