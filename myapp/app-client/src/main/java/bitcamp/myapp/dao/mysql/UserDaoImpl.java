...
public class UserDaoImpl implements UserDao {
    ...
    public User findBy(int no) throws Exception {
        try (PreparedStatement stmt = con.prepareStatement("select user_id, name, email, tel from myapp_users where user_id=?")) {
    ...
    public boolean update(User user) throws Exception {
        try (PreparedStatement stmt = con.prepareStatement("update myapp_users set name=?, email=?, pwd=sha1(?), tel=? where user_id=?")) {
    ...
    public User findByEmailAndPassword(String email, String password) throws Exception {
        try (// SQL을 서버에 전달할 객체 준비
             PreparedStatement stmt = con.prepareStatement("select user_id, name, email, tel from myapp_users where email = ? and pwd = sha1(?)")) {
