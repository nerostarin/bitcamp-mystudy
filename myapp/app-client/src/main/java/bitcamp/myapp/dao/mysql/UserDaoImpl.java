...
public class UserDaoImpl implements UserDao {
        ...
        try (Statement stmt = con.createStatement()) {
            stmt.executeUpdate(String.format(
                    "insert into myapp_users(name, email, pwd, tel)"
                            + "values('%s','%s',sha1('%s'),'%s')",
                    user.getName(), user.getEmail(), user.getPassword(), user.getTel()));
            return true;
        }
    }
    ...
    public List<User> list() throws Exception {
        try (Statement stmt = con.createStatement(); ResultSet rs = stmt.executeQuery("select * from myapp_users order by user_id asc")) {
