...
public class BoardViewServlet extends GenericServlet {
    ...
    public void service(ServletRequest req, ServletResponse res) throws ServletException, IOException {
        ...
        try {
            ...
            out.printf("내용:<input name='content' type='text' value='%s'><br>\n", board.getContent());
            out.printf("작성일:<input readonly type='text' value='%1$tY-%1$tm-%1$td %1$tH:%1$tM:%1$tS'><br>\n", board.getCreatedDate());
            out.printf("조회수:<input readonly type='number' value='%s'><br>\n", board.getViewCount());
            out.printf("작성자:<input readonly type='text' value='%s'><br>\n", board.getWriter().getName());
            out.println("<button>변경</button>");
            out.printf("<button type='button' onclick='location.href=\"/project/delete?no=%d\"'>삭제</button>", board.getNo());
            out.println("</form>");
